			...
			if err != nil {
				...
				if len(key) > 0 {
					...
					if !validHash {
						...
						m.App.Session.Put(r.Context(), "error", "You been logged out from another device")
						next.ServeHTTP(rw, r)
					} else {
					...
					// key length is 0, so it's probably a left over cookie (user has not closed browser)
					m.deleteRememberCookie(rw, r)
					next.ServeHTTP(rw, r)
				}
			}
